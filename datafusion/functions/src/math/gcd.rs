...
use log::info;
use num_traits::{CheckedNeg, Signed, Unsigned};
        ...
        let arr: ArrayRef = match (left.data_type(), right.data_type()) {
            ...
            (
                ...
                calculate_binary_decimal_math::<
                    ...
                    Decimal32Type,
                    _,
                >(&left, right, euclid_gcd, *precision, *scale)?
            }
            (
            ...
            (lhs @ DataType::Decimal128(precision, scale), rhs @ DataType::Int64)
                if *lhs == rhs =>
            {
                calculate_binary_decimal_math::<
                    Decimal128Type,
                ...
                >(
                    &left,
                    right,
                    |arg0: i128, arg1: i64| euclid_gcd(arg0, arg1 as i128),
                    *precision,
                    *scale,
                )?
            }
    ...
    T: ArrowNativeTypeOp + RemAssign + Signed + CheckedNeg,
{
    ...
    let ret = euclid_gcd_unsigned(a, b);
    log::info!("euclid_gcd_unsigned a={a:?} b={b:?} -> {ret:?}");
    ret
}
...
fn euclid_gcd_with_unsigned(a: i64, b: i64) -> Result<i64, ArrowError> {
    ...
    let r = euclid_gcd_unsigned::<u64>(a, b)?;
...
mod tests {
    ...
    fn test_decimal_scalar() {
        let arg_fields = vec![
            ...
            Field::new("b", DataType::Decimal128(32, 0), true).into(),
        ];
        let args = ScalarFunctionArgs {
            args: vec![
        ...
        match result {
            ColumnarValue::Array(arr) => {
                ...
                assert_eq!(*arr.data_type(), DataType::Decimal128(38, 0));
            }
            ColumnarValue::Scalar(_) => {
                panic!("Expected an array value")
            }
    ...
    fn test_decimal_array_scalar() {
        let arg_fields = vec![
            Field::new("a", DataType::Decimal128(38, 0), true).into(),
            Field::new("b", DataType::Decimal128(38, 0), true).into(),
        ];
        let args = ScalarFunctionArgs {
            args: vec![
        ...
        match result {
            ColumnarValue::Array(arr) => {
                ...
                assert_eq!(ints.value(0), i128::from(1));
                assert_eq!(ints.value(0), i128::from(3));
                // Signature stays the same as input
                assert_eq!(*arr.data_type(), DataType::Decimal128(38, 0));
            }
            ColumnarValue::Scalar(_) => {
